...
PROPERTY_DEFAULTS = {
    ...
    '2': '0',
    '3': 'False',
    '4': 'mathutils.Vector((0., 0., 0.))',
    '5': 'mathutils.Color((.5, .5, .5))',
    '6': 'mathutils.Vector((.5, .5, .5, 1.0))',
    '7': 'bpy.types.Object',
    '8': 'bpy.types.Collection',
    '9': 'bpy.types.Material',
    '10': 'bpy.types.Mesh',
    '11': 'bpy.types.NodeTree',
    '12': 'bpy.types.Action',
    '13': 'bpy.types.Text',
    '14': 'bpy.types.Sound',
    '15': 'bpy.types.Image',
    '16': 'bpy.types.VectorFont'
}

VECTOR_PROPERTY_TYPES = {'4', '5', '6'}
...
__NL_CELLS__
        owner["IGNLTree___NL_TREE_NAME__"] = network
        network._owner = game_object
        network.setup()
        network.stopped = not owner.get('NL____NL_TREE_NAME__')
...
class __NL_TREE_NAME__(bge.types.KX_PythonComponent):
    args = OrderedDict([
        ("OnlyRunAtStartup", False),
        ("ExecutionCondition", "")__NL_PROPERTIES__
    ])

    def start(self, args):
        self.logictree = __NL_TREE_NAME___Tree(
            self.object,
            component=self,
...
def get_tree(obj):
    return __NL_TREE_NAME___Tree(obj)
...
def _render_template(tree_name, imports, cells, properties, prop_defs):
    return MODULE_TEMPLATE.replace('__NL_TREE_NAME__', tree_name).replace(
        '__NL_IMPORTS__', imports
    ).replace(
        '__NL_CELLS__', cells
    ).replace(
        '__NL_PROPERTIES__', properties
    ).replace(
        '__NL_PROP_DEFS__', prop_defs
    )
...
class TreeCodeGenerator(object):
    ...
    def write_code_for_tree(self, tree):
        ...
        text = _render_template(tree_name, imports, text, properties, prop)
    ...
    def get_properties(self, tree):
        ...
        for prop in tree.properties:
            if prop.value_type == '17':
                items = [f'"{item.strip()}"' for item in prop.value.split(',') if item.strip()]
                val = '{' + ', '.join(items) + '}'
            else:
                val = PROPERTY_DEFAULTS.get(prop.value_type, 'None')
            text += f',\n        ("{prop.name}", {val})'
        return text
    ...
    def get_property_defs(self, tree):
        text = ''
        for prop in tree.properties:
            getter = f'Vector(args["{prop.name}"])' if prop.value_type in VECTOR_PROPERTY_TYPES else f'args["{prop.name}"]'
    ...
    def _write_tree(self, tree):
        ...
        for uid in range(0, cell_uid):
            ...
            if tree_node is None or cell_varname is None:
                utils.error(f'UIDMap is missing an entry for uid {uid}. Skipping node.')
                continue
    ...
    def _sort_cellvarnames(self, node_cellvar_list, uid_map):
        ...
        while available_cells:
            now = time()
            if now - start > 4:
                msg = 'Timeout Error while sorting cells. Check tree for unlinked Reroutes or other issues.'
                utils.error(msg)
            ...
            for cell_name in available_cells:
                ...
                if node is None:
                    utils.error(f'UIDMap is missing a node for varname {cell_name}. Skipping.')
