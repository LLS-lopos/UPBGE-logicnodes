...
class LogicNodeTree(NodeTree):
    ...
    def mark_invalid_links(self):
        ...
        _invalid_links_timers.pop(self, None)
        try:
            ...
            error(f'Could not validate links in tree "{self.name}": {e}')
    ...
    def update(self):
        timer = _invalid_links_timers.get(self)
        if timer:
            try:
                bpy.app.timers.unregister(timer)
            except Exception:
                pass
        _invalid_links_timers[self] = bpy.app.timers.register(self.mark_invalid_links)
        start = time()
