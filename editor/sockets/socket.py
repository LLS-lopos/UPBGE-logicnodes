...
BL_SOCKET_TYPES = {
    SOCKET_TYPE_GENERIC: 'VALUE',
    SOCKET_TYPE_CONDITION: 'MATERIAL',
    2: 'VALUE',
    SOCKET_TYPE_INT: 'INT',
    4: 'INT',
    SOCKET_TYPE_FLOAT: 'VALUE',
    6: 'VALUE',
    SOCKET_TYPE_STRING: 'STRING',
    SOCKET_TYPE_BOOL: 'BOOLEAN',
    SOCKET_TYPE_VECTOR: 'VECTOR',
    SOCKET_TYPE_MATRIX: 'VECTOR',
    SOCKET_TYPE_COLOR: 'RGBA',
    SOCKET_TYPE_OBJECT: 'OBJECT',
    13: 'INT',
    SOCKET_TYPE_DATABLOCK: 'TEXTURE',
    SOCKET_TYPE_UI: 'GEOMETRY',
    SOCKET_TYPE_COLLECTION: 'COLLECTION',
    SOCKET_TYPE_VALUE: 'STRING',
    SOCKET_TYPE_TEXT: 'TEXTURE',
    SOCKET_TYPE_SOUND: 'TEXTURE',
    SOCKET_TYPE_IMAGE: 'TEXTURE',
    SOCKET_TYPE_FONT: 'TEXTURE',
    SOCKET_TYPE_ACTION: 'TEXTURE',
    SOCKET_TYPE_ARMATURE: 'TEXTURE',
    SOCKET_TYPE_LIST: 'INT',
    SOCKET_TYPE_DICTIONARY: 'BOOLEAN',
    SOCKET_TYPE_MATERIAL: 'TEXTURE',
    SOCKET_TYPE_NODETREE: 'TEXTURE',
    SOCKET_TYPE_MESH: 'GEOMETRY',
    SOCKET_TYPE_PYTHON: 'GEOMETRY',
    SOCKET_TYPE_SCENE: 'MATERIAL'
}
...
class NodeSocketLogic:
    ...
    def get_from_socket(self):
        ...
        while isinstance(from_socket.node, NodeReroute):
            if from_socket.node in visited:
                warn(f'Cycle detected while resolving reroute chain on socket "{self.name}"!')
                return from_socket
            visited.add(from_socket.node)
    ...
    def init(self):
        self.type = BL_SOCKET_TYPES.get(self.nl_type, 'VALUE')
    ...
    def validate(self, link: NodeLink, from_socket: NodeSocket, nodetree):
        ...
        while isinstance(from_socket.node, NodeReroute) and len(from_socket.node.inputs[0].links) > 0:
            if from_socket.node in visited:
                warn(f'Cycle detected while validating link on socket "{self.name}"!')
                link.is_valid = False
                return
            visited.add(from_socket.node)
            from_socket = from_socket.node.inputs[0].links[0].from_socket
