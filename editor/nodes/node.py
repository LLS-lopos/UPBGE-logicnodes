...
class LogicNode:
    ...
    def rebuild(self, name_map={}):
        ...
        for i in self.inputs:
            ident = getattr(i, 'identifier', '') or name_map.get(i.name, i.name)
            ipts[ident] = [link.from_socket for link in i.links]
        opts = {}
        for i in self.outputs:
            ident = getattr(i, 'identifier', '') or name_map.get(i.name, i.name)
            opts[ident] = [link.to_socket for link in i.links]
        self.inputs.clear()
        self.outputs.clear()
        self.init(bpy.context)
        for i in self.inputs:
            ident = getattr(i, 'identifier', '') or i.name
            linked = ipts.get(ident)
            if linked:
                for socket in linked:
                    self.tree.links.new(socket, i)
        for i in self.outputs:
            ident = getattr(i, 'identifier', '') or i.name
            linked = opts.get(ident)
            if linked:
                for socket in linked:
    ...
    def get_linked_value(
        ...
        while isinstance(output_node, NodeReroute):
            if output_node in visited:
                error(f'Cycle detected while resolving reroute chain from node "{self.name}"!')
                return None
        ...
        if output_node_varname is None:
            error(f'Node "{output_node.name}" is not registered in the tree! Check for muted or invalid nodes.')
            return None
